...
import { IndoorAqi_All } from '../../util/getPost';
...
const FrontPageIndoor = ({ navigation }) => {

    const [catergoryIndex, setCategoryIndex] = React.useState(0);
    const [chartTabIndex, setChartTab] = React.useState(0);
    const [latest_indoor_aqi, setLatestIndoor_Aqi] = React.useState();
    const [daily_indoor_aqi, setDailyIndoor_Aqi] = React.useState();
    const [week_indoor_aqi, setWeekIndoor_Aqi] = React.useState();
    const [month_indoor_aqi, setMonthIndoor_Aqi] = React.useState();
    ...
    async function fetch_all_aqi (){
        ...
        .then((data) => {
            ...
            let five_Data = data.filter(function(el,index){
              ...
              five_Data.map((val) => {
                historyValue = {"PM1": val.pm1, "CO2": val.co2, "VOC": val.voc, "PM25": val.pm25, "PM10": val.pm10, "Temperature":val.temp,"Humidity":val.humd},
            ...
            let arrayLatestData = [
                {"label":"iaq_score","value":latestData.iaqScore, "color":'#FF6347'},
                {"label":"PM1","value":latestData.pm1, "color":'#00CED1'},
                {"label":"CO2","value":latestData.co2, "color":'#FFA500'},
                {"label":"VOC","value":latestData.voc, "color":'#800080'},
                {"label":"PM25","value":latestData.pm25, "color":'#8A2BE2'},
                {"label":"PM10","value":latestData.pm10, "color":'#32CD32'},
                {"label":"Temperature","value":latestData.temp, "color":'#32CD32'},
                {"label":"Humidity","value":latestData.humd, "color":'#32CD32'},
                {"label":"Timestamps","value":latestData.timestamps, "color":'#32CD32'},
            ]
            // console.log(arrayLatestData)
            let array_daily_chart = [
                {"label":"PM1","value":latestData.pm1, "color":'#00CED1'},
                {"label":"CO2","value":latestData.co2, "color":'#FFA500'},
                {"label":"VOC","value":latestData.voc, "color":'#800080'},
                {"label":"PM25","value":latestData.pm25, "color":'#8A2BE2'},
            ...
            setDailyIndoor_Aqi(array_daily_chart)
            setLatestIndoor_Aqi(arrayLatestData)

            // data per week
            let oneWeekData = data.slice(0,7)
            let weekData={}
            let weekArray = []
            oneWeekData.map((item, index) => {
                weekData = [
                    { color: "rgba(255, 99, 71, 1)", index: item.timestamp, data: { label: "PM1", value: item.pm1 } },
                    { color:"rgba(0, 206, 209, 1)", index:item.timestamp, data:{label:"CO2","value":item.co2,}},
                    {color : "rgba(255, 165, 0, 1)",index:item.timestamp,data:{label:"VOC","value":item.voc}},
                        {color : "rgba(128, 0, 128, 1)", index:item.timestamp, data:{"label":"PM25","value":item.pm25}, },
                        {color: "rgba(138, 43, 226, 1)", index:item.timestamp,data:{"label":"PM10","value":item.pm10}, },
                  ];
                  
                weekArray.push(weekData)
            }
            )
            // console.log('week array', oneWeekData)
            ...
            setWeekIndoor_Aqi(groupedData)
            
            // data per month
            let oneMonthData = data.slice(0,30)
            let monthData={}
            let monthArray = []
            ...
            setMonthIndoor_Aqi(groupedDataMonth)
            ...
            >
                ...
                >
                    <View style={{ flexDirection: 'row', justifyContent:'center', alignContent:'center' }}>
                        <View style={{}}>
                            <Text style={{ fontSize: 10, alignItems:'center',fontWeight:'bold',paddingBottom:5 }}>INDOOR AIR</Text>
                            <Text style={{ alignItems:'center',textAlign:'center', fontSize:20,fontWeight:'bold',paddingTop:10 }}>{latest_indoor_aqi[0].value}</Text>
    ...
    function renderRequirements() {
        return (
            <View style={{ flex: 2.5, marginTop: SIZES.padding, paddingHorizontal: SIZES.padding, justifyContent: 'space-around' }}>
                ...
                <RequirementDetail
                    icon={icons.sun}
                    label="PM1 (Particulate Matter)"
                    detail={latest_indoor_aqi[1].value}
                />
                <RequirementDetail
                    icon={icons.drop}
                    label="CO2 (Carbon Dioxide)"
                    detail={latest_indoor_aqi[2].value}
                />
                <RequirementDetail
                    icon={icons.temperature}
                    label="VOC (Volatile Organic Compounds)"
                    detail={latest_indoor_aqi[3].value}
                />
                <RequirementDetail
                    icon={icons.garden}
                    label="PM25 (Particulate Matter)"
                    detail={latest_indoor_aqi[4].value}
                />
                <RequirementDetail
                    icon={icons.seed}
                    label="PM.10 (Particulate Matter)"
                    detail={latest_indoor_aqi[5].value}
                />
                <RequirementDetail
                    icon={icons.seed}
                    label="Temperature"
                    detail={latest_indoor_aqi[6].value}
                />
                <RequirementDetail
                    icon={icons.seed}
                    label="Humidity"
                    detail={latest_indoor_aqi[7].value}
                />
            </View>
        )
    }
    ...
    DailyChart = () => {
        return (
            <View>
                 <LineChart
                   data={{
                    labels: daily_indoor_aqi.map(item => item.label),
                    datasets: [
                        {
                        data: daily_indoor_aqi.map(item => item.value),
                    ...
                    withOuterLines={true} // Set this prop to false to avoid the TypeError
                    withLegend={true}
                    ...
                    backgroundGradientFrom: "#3C2317",
                    backgroundGradientTo: "#251B37",
    ...
    WeeklyChart = () => {
        const labels = Object.keys(week_indoor_aqi);
        // console.log('label', labels)
      
        const data = [];

        for (const dayKey in week_indoor_aqi) {
        const dayData = week_indoor_aqi[dayKey];
        ...
        const chartData = data.map(item => ({
            data: item.value,
            color: (opacity = 1) => item.label === "PM1" ? `rgba(255, 99, 71, ${opacity})` : item.label === "CO2" ? "rgba(86, 43, 8, 1)" : item.label === "VOC" ? "rgba(255, 30, 30, 1)":item.label === "PM25" ? "rgba(139, 188, 204, 1)" :`rgba(50, 205, 50, ${opacity})`
        }));
    ...
    const MonthlyChart = () => {
        const labels = Object.keys(month_indoor_aqi);
      
        const data = [];

        for (const monthKey in month_indoor_aqi) {
        const monthData = month_indoor_aqi[monthKey];
          ...
          const Item = ({item}) => (
            ...
            <View style={[styles.cardList]}>
              ...
              <View style={{flexDirection:'column'}}>
                <View style={{flexDirection:'row'}}>
                    <View>
                    <Text style={[styles.title,{color:'#060047'}]}>PM1: </Text>
                    </View>
                    <View>
                    <Text style={[styles.score,{color:'#060047'}]}>{item.PM1}</Text>
                    </View>
                </View>
                <View style={{flexDirection:'row'}}>
                    <View>
              ...
              <View style={{flexDirection:'column'}}>
                <View style={{flexDirection:'row'}}>
                <View>
                    <Text style={[styles.title,{color:'#060047'}]}>VOC: </Text>
                    </View>
                    <View>
                    <Text style={[styles.score,{color:'#060047'}]}>{item.VOC}</Text>
                    </View>
                </View>
                <View style={{flexDirection:'row'}}>
                    <View>
                        <Text style={[styles.title,{color:'#060047'}]}>Temperature: </Text>
                        </View>
                        <View>
                ...
                <View style={{flexDirection:'row'}}>
                    <View>
                        <Text style={[styles.title,{color:'#060047'}]}>Humidity: </Text>
                        </View>
                        <View>
...
export default FrontPageIndoor;